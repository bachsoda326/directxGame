...
*/
void CGame::Draw(float x, float y, LPDIRECT3DTEXTURE9 texture, int left, int top, int right, int bottom, D3DXVECTOR2 transform, int alpha)
{	
	...
	D3DXMATRIX oldMatrix, mMatrix;
	D3DXVECTOR2 inTranslation = transform;
	D3DXMatrixTransformation2D(&mMatrix, 0, 0, 0, 0, 0, &inTranslation);
...
void CGame::InitKeyboard(LPKEYEVENTHANDLER handler)
{
	...
	hr = didv_keyboard->SetCooperativeLevel(hWnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
	...
	hr = didv_keyboard->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph);
...
void CGame::ProcessKeyboard()
{
	...
	hr = didv_keyboard->GetDeviceState(sizeof(keyStates), keyStates);
	if (FAILED(hr))
	{
	...
	hr = didv_keyboard->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), keyEvents, &dwElements, 0);
	if (FAILED(hr))
	{
